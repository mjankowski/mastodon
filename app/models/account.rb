...
class Account < ApplicationRecord
  ...
  with_options unless: :remote? do
    validates :username, format: { with: /\A[a-z0-9_]+\z/i }, length: { maximum: USERNAME_LENGTH_LIMIT }, if: -> { will_save_change_to_username? && actor_type != 'Application' }
    validates_with UnreservedUsernameValidator, if: -> { will_save_change_to_username? && actor_type != 'Application' }
