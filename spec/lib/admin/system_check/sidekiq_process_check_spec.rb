...
describe Admin::SystemCheck::SidekiqProcessCheck do
  ...
  describe 'message' do
    it 'sends values to message instance' do
      allow(Admin::SystemCheck::Message).to receive(:new).with(:solid_queue_process_check, 'default, push, mailers, pull, scheduler, ingress')

      check.message

      expect(Admin::SystemCheck::Message).to have_received(:new).with(:solid_queue_process_check, 'default, push, mailers, pull, scheduler, ingress')
    end
  end
end
