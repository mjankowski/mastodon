...
describe OStatus::TagManager do
  describe '#unique_tag' do
    it 'returns a unique tag' do
      expect(described_class.instance.unique_tag(Time.utc(2000), 12, 'Status')).to eq 'tag:localhost:3000,2000-01-01:objectId=12:objectType=Status'
    end
  end

  describe '#unique_tag_to_local_id' do
    it 'returns the ID part' do
      expect(described_class.instance.unique_tag_to_local_id('tag:localhost:3000,2000-01-01:objectId=12:objectType=Status', 'Status')).to eql '12'
    end
    ...
    it 'returns nil if it is not expected type' do
      expect(described_class.instance.unique_tag_to_local_id('tag:localhost:3000,2000-01-01:objectId=12:objectType=Block', 'Status')).to be_nil
    end

    it 'returns nil if it does not have object ID' do
      expect(described_class.instance.unique_tag_to_local_id('tag:localhost:3000,2000-01-01:objectType=Status', 'Status')).to be_nil
    end
  end

  describe '#local_id?' do
    it 'returns true for a local ID' do
      expect(described_class.instance.local_id?('tag:localhost:3000;objectId=12:objectType=Status')).to be true
    end
  ...
  describe '#uri_for' do
    ...
    context 'when person object' do
      ...
      it 'returns the URL for account' do
        ...
        expect(subject).to eq 'http://localhost:3000/users/alice'
      end
    end
  end
end
