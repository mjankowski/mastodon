    ...
    context 'when webfinger fails and host meta is used' do
      ...
      context 'when host meta succeeds' do
        ...
        before do
          ...
          stub_request(:get, 'https://example.com/.well-known/nonStandardWebfinger?resource=acct:alice@example.com').to_return(body: JSON.dump(webfinger), headers: { 'Content-Type': 'application/jrd+json' })
        end
