  ...
  describe 'GET /api/v1/accounts/:id/featured_tags' do
    ...
    it 'returns the expected tags', :aggregate_failures do
      ...
      expect(body_as_json).to contain_exactly(a_hash_including({
        name: 'bar',
        url: "http://localhost:3000/@#{account.username}/tagged/bar",
      }), a_hash_including({
        name: 'foo',
        url: "http://localhost:3000/@#{account.username}/tagged/foo",
      }))
    end
    ...
    context 'when the account is remote' do
      it 'returns the expected tags', :aggregate_failures do
        ...
        expect(body_as_json).to contain_exactly(a_hash_including({
          name: 'bar',
          url: "http://localhost:3000/@#{account.pretty_acct}/tagged/bar",
        }), a_hash_including({
          name: 'foo',
          url: "http://localhost:3000/@#{account.pretty_acct}/tagged/foo",
        }))
      end
    end
  end
