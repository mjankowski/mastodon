...
RSpec.describe NotificationMailer do
  ...
  shared_examples 'standard headers' do |type|
    it 'renders the email' do
      expect(mail)
        ...
        .and(have_header('List-ID', "<#{type}.alice.localhost:3000>"))
        .and(have_header('List-Unsubscribe', %r{<http://localhost:3000/unsubscribe\?token=.+>}))
  ...
  shared_examples 'thread headers' do
    it 'renders the email with conversation thread headers' do
      conversation_header_regex = /<conversation-\d+.\d\d\d\d-\d\d-\d\d@localhost:3000>/
      expect(mail)
        .to be_present
