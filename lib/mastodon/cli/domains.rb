  ...
  class Domains < Base
    ...
    def purge(*domains)
      ...
      if options[:limited_federation_mode]
        account_scope = non_allowed_domain_accounts
        emoji_scope = non_allowed_domain_emojis
      else
    ...
    def non_allowed_domain_accounts
      remote_accounts.where.not(domain: DomainAllow.select(:domain))
    end

    def non_allowed_domain_emojis
      CustomEmoji.remote.where.not(domain: DomainAllow.select(:domain))
    end
