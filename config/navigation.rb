...
SimpleNavigation::Configuration.run do |navigation|
  navigation.items do |n|
    ...
    n.item :preferences, safe_join([material_symbol('settings'), t('settings.preferences')]), settings_preferences_path, if: -> { current_user.functional? }, unless: -> { self_destruct? } do |s|
    ...
    n.item :relationships, safe_join([material_symbol('groups'), t('settings.relationships')]), relationships_path, if: -> { current_user.functional? }, unless: -> { self_destruct? } do |s|
    ...
    n.item :filters, safe_join([material_symbol('filter_alt'), t('filters.index.title')]), filters_path, highlights_on: %r{/filters}, if: -> { current_user.functional? }, unless: -> { self_destruct? }
    n.item :statuses_cleanup, safe_join([material_symbol('history'), t('settings.statuses_cleanup')]), statuses_cleanup_path, if: -> { current_user.functional_or_moved? }, unless: -> { self_destruct? }
