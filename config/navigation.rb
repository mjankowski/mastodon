...
SimpleNavigation::Configuration.run do |navigation|
  navigation.items do |nav|
    ...
    nav.item :preferences, safe_join([material_symbol('settings'), t('settings.preferences')]), settings_preferences_path, if: -> { current_user.functional? }, unless: -> { self_destruct? } do |s|
    ...
    nav.item :relationships, safe_join([material_symbol('groups'), t('settings.relationships')]), relationships_path, if: -> { current_user.functional? }, unless: -> { self_destruct? } do |s|
    ...
    nav.item :security, safe_join([material_symbol('account_circle'), t('settings.account')]), edit_user_registration_path do |s|
      ...
      s.item :authorized_apps, safe_join([material_symbol('list_alt'), t('settings.authorized_apps')]), oauth_authorized_applications_path, unless: -> { self_destruct? }
    end

    nav.item :data, safe_join([material_symbol('cloud_sync'), t('settings.import_and_export')]), settings_export_path do |s|
      s.item :import, safe_join([material_symbol('cloud_upload'), t('settings.import')]), settings_imports_path, highlights_on: %r{/settings/imports}, if: -> { current_user.functional? }, unless: -> { self_destruct? }
    ...
    nav.item :trends, safe_join([material_symbol('trending_up'), t('admin.trends.title')]), admin_trends_statuses_path, if: -> { current_user.can?(:manage_taxonomies) }, unless: -> { self_destruct? } do |s|
    ...
    nav.item :moderation, safe_join([material_symbol('gavel'), t('moderation.title')]), nil, if: -> { navigation_shows_moderation? }, unless: -> { self_destruct? } do |s|
    ...
    nav.item :admin, safe_join([material_symbol('manufacturing'), t('admin.title')]), nil, if: -> { navigation_shows_admin? }, unless: -> { self_destruct? } do |s|
